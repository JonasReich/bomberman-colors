...
std::pair<unsigned int, unsigned int> CUnitManager::GetHeroPosition(unsigned int iPlayer) const
{
	unsigned int iUnit;
	for (iUnit = 0; iUnit < m_PlayerUnits[iPlayer].size(); iUnit++)
		if (m_PlayerUnits[iPlayer][iUnit].m_Type == UNIT_TYPE_HERO)
			return std::make_pair(m_PlayerUnits[iPlayer][iUnit].m_PosX, m_PlayerUnits[iPlayer][iUnit].m_PosY);
